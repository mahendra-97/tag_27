    ...
    def post(self, request):
        ...
        try:
            ...
            if action == 'assign':
                ...
                vm_ids = request.data.getlist('vm_ids')

                tag = get_object_or_404(TagsModel, tag_id=tag_id)
...
class VMs(APIView):
    ...
    def put(self, request):
        ...
        #         data = {'status':'error','error_code': 100, 'message': _('VM id is required')}
        #         return JsonResponse(data)
...
class Users(APIView):
    def get(self, request):
        ...
        users = [{'user_id': user.user_id, 'user_name': user.user_name} for user in queryset]
